...
import com.cookiecraftmods.mta.client.compat.ClickableWidgetExtension;
...
public class TrafficWidgetMap extends ClickableWidgetExtension implements IGui {
	...
	private void drawEntryLabel(GraphicsHolder graphicsHolder, ClientTrafficDashboardEntry entry) {
		drawFromWorldCoords(entry.blockPos().getX() + 0.5D, entry.blockPos().getZ() + 0.5D, (x, z) -> {
			...
			IDrawing.drawStringWithFont(graphicsHolder.pose(), graphicsHolder.bufferSource(), shortLabel(entry), 0, -TEXT_HEIGHT, GraphicsHolder.getDefaultLight());
			graphicsHolder.pop();
		});
	}
	...
	private void drawIntersectionLabel(GraphicsHolder graphicsHolder, ClientTrafficIntersectionEntry intersection) {
		drawFromWorldCoords(intersection.centerX() + 0.5D, intersection.centerZ() + 0.5D, (x, z) -> {
			graphicsHolder.push();
			graphicsHolder.translate(getX2() + x.floatValue(), getY2() + z.floatValue(), 0);
			IDrawing.drawStringWithFont(graphicsHolder.pose(), graphicsHolder.bufferSource(), (intersection.level() == TrafficIntersectionLevel.TRAIN ? "T " : "C ") + intersection.nodes().size(), 0, -TEXT_HEIGHT, GraphicsHolder.getDefaultLight());
			graphicsHolder.pop();
		});
	}
	...
	private void drawPlayerSymbol(GuiDrawing guiDrawing, LocalPlayer clientPlayer) {
	...
	private void drawTextureFromWorldCoords(GuiDrawing guiDrawing, ResourceLocation texture, double posX1, double posZ1, double posX2, double posZ2) {
