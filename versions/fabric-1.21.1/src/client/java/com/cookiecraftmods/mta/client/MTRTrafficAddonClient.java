...
public class MTRTrafficAddonClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ItemProperties.register(ModItems.TRAFFIC_SPAWN_CONNECTOR, ResourceLocation.fromNamespaceAndPath("mtr", "selected"), (stack, level, entity, seed) -> stack.get(DataComponentTypes.START_POS.get()) != null ? 1.0F : 0.0F);
		ItemProperties.register(ModItems.TRAFFIC_DESPAWN_CONNECTOR, ResourceLocation.fromNamespaceAndPath("mtr", "selected"), (stack, level, entity, seed) -> stack.get(DataComponentTypes.START_POS.get()) != null ? 1.0F : 0.0F);
		ItemProperties.register(ModItems.SIGNAL_PATH_BLOCKER_CONNECTOR, ResourceLocation.fromNamespaceAndPath("mtr", "selected"), (stack, level, entity, seed) -> stack.get(DataComponentTypes.START_POS.get()) != null ? 1.0F : 0.0F);
		ItemProperties.register(ModItems.MTA_PATH_BLOCKER_CONNECTOR, ResourceLocation.fromNamespaceAndPath("mtr", "selected"), (stack, level, entity, seed) -> stack.get(DataComponentTypes.START_POS.get()) != null ? 1.0F : 0.0F);

		// NETWORK PACKET LISTENERS - odbieranie danych z serwera
		// Debug snapshot - pozycje pojazdów dla debugowania
		ClientPlayNetworking.registerGlobalReceiver(TrafficNetworking.DEBUG_SNAPSHOT_PACKET_ID, (payload, context) -> {
			...
			context.client().execute(() -> ClientTrafficDebugState.replace(sequence, snapshots));
		});

		// DASHBOARD PACKET - informacje o punktach spawn i skrzyżowaniach
		ClientPlayNetworking.registerGlobalReceiver(TrafficDashboardNetworking.SNAPSHOT_PACKET_ID, (payload, context) -> {
			final FriendlyByteBuf buffer = payload.buffer();
			final int count = buffer.readVarInt();
			...
			context.client().execute(() -> TrafficDashboardClient.openOrUpdate(entries, intersections));
		});

		// LIGHT BINDING MENU PACKET - otwarcie menu bindowania świateł
		ClientPlayNetworking.registerGlobalReceiver(TrafficLightBindingNetworking.OPEN_MENU_PACKET_ID, (payload, context) -> {
			final FriendlyByteBuf buffer = payload.buffer();
			...
			context.client().execute(() -> context.client().setScreen(new TrafficLightBindingScreen(blockPos, intersections)));
		});

		ClientPlayNetworking.registerGlobalReceiver(RoadSignNetworking.OPEN_EDITOR_PACKET_ID, (payload, context) -> {
			final FriendlyByteBuf buffer = payload.buffer();
			final BlockPos blockPos = buffer.readBlockPos();
			...
			context.client().execute(() -> context.client().setScreen(new RoadSignEditScreen(blockPos, baseId, lines, textColor, signWidth, signHeight, backgroundColor, edgeColor, imageId)));
		});
