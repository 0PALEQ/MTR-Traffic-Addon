...
import com.cookiecraftmods.mta.client.compat.ButtonWidgetExtension;
import com.cookiecraftmods.mta.client.compat.ClickableWidget;
import com.cookiecraftmods.mta.client.compat.GraphicsHolder;
import com.cookiecraftmods.mta.client.compat.GuiDrawing;
import com.cookiecraftmods.mta.client.compat.ScreenExtension;
import com.cookiecraftmods.mta.client.compat.TextCase;
import com.cookiecraftmods.mta.client.compat.TextFieldWidgetExtension;
import com.cookiecraftmods.mta.client.compat.TextHelper;
...
public class TrafficDashboardScreen extends ScreenExtension implements IGui {
	...
	private static final ResourceLocation KOFI_ICON = ResourceLocation.fromNamespaceAndPath(MTRTrafficAddon.MOD_ID, "textures/gui/social/kofi.png");
	private static final ResourceLocation DISCORD_ICON = ResourceLocation.fromNamespaceAndPath(MTRTrafficAddon.MOD_ID, "textures/gui/social/discord.png");
	...
	private void drawButtonIcon(GraphicsHolder gh, ResourceLocation texture, ButtonWidgetExtension button, int sourceWidth, int sourceHeight) {
		final int padding = 3;
	...
	private void sendUpdate(String action, int delta, String value) {
		...
		ClientPlayNetworking.send(MtaPacketPayload.fromBuffer(TrafficDashboardNetworking.UPDATE_PACKET_ID, buf));
	}
	...
	private void sendIntersectionUpdate(String action, int delta, String value) {
		...
		buf.writeBoolean(value != null); if (value != null) buf.writeUtf(value);
		ClientPlayNetworking.send(MtaPacketPayload.fromBuffer(TrafficDashboardNetworking.INTERSECTION_UPDATE_PACKET_ID, buf));
	}

	private void sendCreateIntersection(BlockPos a, BlockPos b) {
		final FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
		buf.writeBlockPos(a); buf.writeBlockPos(b);
		ClientPlayNetworking.send(MtaPacketPayload.fromBuffer(TrafficDashboardNetworking.INTERSECTION_CREATE_PACKET_ID, buf));
	}

	private void sendRefresh()       { ClientPlayNetworking.send(MtaPacketPayload.fromBuffer(TrafficDashboardNetworking.REFRESH_PACKET_ID, new FriendlyByteBuf(Unpooled.buffer()))); }
	private void sendClearVehicles() { ClientPlayNetworking.send(MtaPacketPayload.fromBuffer(TrafficDashboardNetworking.CLEAR_VEHICLES_PACKET_ID, new FriendlyByteBuf(Unpooled.buffer()))); }
