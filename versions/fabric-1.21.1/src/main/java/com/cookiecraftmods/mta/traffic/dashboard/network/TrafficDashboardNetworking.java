...
public final class TrafficDashboardNetworking {
	public static final CustomPacketPayload.Type<MtaPacketPayload> SNAPSHOT_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_snapshot");
	public static final CustomPacketPayload.Type<MtaPacketPayload> UPDATE_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_update");
	public static final CustomPacketPayload.Type<MtaPacketPayload> INTERSECTION_UPDATE_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_intersection_update");
	public static final CustomPacketPayload.Type<MtaPacketPayload> INTERSECTION_CREATE_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_intersection_create");
	public static final CustomPacketPayload.Type<MtaPacketPayload> REFRESH_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_refresh");
	public static final CustomPacketPayload.Type<MtaPacketPayload> CLEAR_VEHICLES_PACKET_ID = MtaPacketPayload.type("traffic_dashboard_clear_vehicles");
	...
	public static void initialize() {
		...
		MtaPacketPayload.registerS2C(SNAPSHOT_PACKET_ID);
		MtaPacketPayload.registerC2S(UPDATE_PACKET_ID);
		MtaPacketPayload.registerC2S(INTERSECTION_UPDATE_PACKET_ID);
		MtaPacketPayload.registerC2S(INTERSECTION_CREATE_PACKET_ID);
		MtaPacketPayload.registerC2S(REFRESH_PACKET_ID);
		MtaPacketPayload.registerC2S(CLEAR_VEHICLES_PACKET_ID);

		ServerPlayNetworking.registerGlobalReceiver(REFRESH_PACKET_ID, (payload, context) -> {
			final ServerPlayer player = context.player();
			context.server().execute(() -> {
				final boolean refreshRequested = TrafficManager.requestNetworkRefresh(player);
				MTRTrafficAddon.LOGGER.info("Traffic dashboard network refresh requested by {}: {}", player.getGameProfile().getName(), refreshRequested ? "accepted" : "unavailable");
				sendSnapshot(player);
			});
		});

		ServerPlayNetworking.registerGlobalReceiver(CLEAR_VEHICLES_PACKET_ID, (payload, context) -> {
			final ServerPlayer player = context.player();
			context.server().execute(() -> {
		...
		ServerPlayNetworking.registerGlobalReceiver(INTERSECTION_CREATE_PACKET_ID, (payload, context) -> {
			final FriendlyByteBuf buffer = payload.buffer();
			final ServerPlayer player = context.player();
		...
		ServerPlayNetworking.registerGlobalReceiver(INTERSECTION_UPDATE_PACKET_ID, (payload, context) -> {
			final FriendlyByteBuf buffer = payload.buffer();
			final ServerPlayer player = context.player();
			...
			context.server().execute(() -> {
				...
				final int refreshedNodes = "find_nodes".equals(action) ? TrafficManager.refreshIntersectionNodesNear(player) : 0;
	...
	public static void sendSnapshot(ServerPlayer player) {
		...
		ServerPlayNetworking.send(player, MtaPacketPayload.fromBuffer(SNAPSHOT_PACKET_ID, buffer));
	}
