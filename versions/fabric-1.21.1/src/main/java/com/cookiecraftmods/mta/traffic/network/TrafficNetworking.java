...
public final class TrafficNetworking {
	public static final CustomPacketPayload.Type<MtaPacketPayload> DEBUG_SNAPSHOT_PACKET_ID = MtaPacketPayload.type("debug_snapshot");
	...
	public static void initialize() {
		...
		MtaPacketPayload.registerS2C(DEBUG_SNAPSHOT_PACKET_ID);
	...
	private static void broadcastReadySnapshots(Collection<ServerPlayer> players) {
		...
		for (ServerPlayer player : players) {
			...
			if (snapshot != null && snapshot.sequence() > LAST_SENT_SEQUENCE.getOrDefault(player.getUUID(), -1L)) {
				...
				try {
					ServerPlayNetworking.send(player, MtaPacketPayload.fromBuffer(DEBUG_SNAPSHOT_PACKET_ID, buffer));
				} finally {
					buffer.release();
				}
